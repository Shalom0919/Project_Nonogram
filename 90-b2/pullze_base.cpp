	...
	for (int i = 0; i < game.rows; i++) {
		...
		for (int j = 0; j < game.cols; j++) {
			if (game.player[i][j] == 1) {
				...
				if (len > 0) {
					game.row_hints_player[i][count++] = len;
					len = 0; // 断开上一段
				}
			}
		}
		...
		game.row_hint_count_player[i] = count; // 段数
	}
	...
	for (int j = 0; j < game.cols; j++) {
		...
		for (int i = 0; i < game.rows; i++) {
			if (game.player[i][j] == 1) {
				...
				if (len > 0) {
					game.col_hints_player[j][count++] = len;
					len = 0; // 断开上一段
				}
			}
		}
		...
		game.col_hint_count_player[j] = count; // 段数
	}
}

bool check(GameState& game)
{
	calculate_player_hints(game);
	for (int i = 0; i < game.rows; i++) {
		if (game.row_hint_count_player[i] != game.row_hint_count[i])
			return false;
		int count = game.row_hint_count_player[i];
		for (int j = 0; j < count; j++) {
			if (game.row_hints_player[i][j] != game.row_hints[i][j])
				return false;
		}
	}

	for (int i = 0; i < game.cols; i++) {
		if (game.col_hint_count_player[i] != game.col_hint_count[i])
			return false;
		int count = game.col_hint_count_player[i];
		for (int j = 0; j < count; j++) {
			if (game.col_hints_player[i][j] != game.col_hints[i][j])
				return false;
		}
	}
		...
		for (int j_in_segment = 0; j_in_segment < 5 && (segment * 5 + j_in_segment) < game.rows; j_in_segment++) {
			...
			for (int i = 0; i < game.cols; i++) {
				...
				if ((game.player[j][i] == 1 && !is_cheat)
					...
					cct_setcolor(COLOR_HYELLOW, COLOR_HBLACK);
					cout << "0"; // 玩家选择的位置正确
					cct_setcolor();
					cout << " ";
				}
				...
				else if (is_cheat && (game.player[j][i] == 0 || game.player[j][i] == -1) && game.grid[j][i] == 1) {
					cout << "0 "; // 没选这个位置
				}
				else {
					cout << "  "; // 未选择的位置显示空格
